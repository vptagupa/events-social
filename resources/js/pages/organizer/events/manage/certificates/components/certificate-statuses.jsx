...
export default function Statuses({ setSearch }) {
    const { certificate_statuses, default_certificate_status } =
        usePage().props;
    const [data, setData] = useState(certificate_statuses);
    const [selected, setSelected] = useState(
        certificate_statuses.filter(
            (s) => s.id == default_certificate_status
        )[0]
    );
    ...
    useEffect(() => {
        setSearch((search) => ({
            ...search,
            certificate_status: selected.id,
        }));
    }, [selected]);
